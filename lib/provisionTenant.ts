...
async function notionPost(apiKey: string, path: string, payload: object): Promise<any> {
...
async function checkSampleExists(apiKey: string, customerDbId: string): Promise<boolean> {
...
export async function seedSampleCustomer(
  ...
  const alreadyExists = await checkSampleExists(notionApiKey, dbIds.customerDbId);
  ...
  // 顧客作成
  await notionPost(notionApiKey, '/pages', {
  ...
  // 体重ログシード（直近9日）
  const weights = [58.0, 57.8, 57.9, 57.6, 57.5, 57.7, 57.4, 57.3, 57.5];
