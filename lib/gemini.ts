...
処理手順（厳守・例外なし）：
1. メモに記載された料理は、写真に写っているかどうかに関係なく、必ず items配列に含める
   - メモ「鶏むね肉100g」と書かれていれば、items に「鶏むね肉 100g」を必ず追加する
   - 写真に全く別の料理が写っていてもメモを無視してはならない
   - メモは「写真に追加して食べた料理」または「写真に映ってない料理」を申告するもの
2. メモの量は、日本食品標準成分表の正確な値で計算する（控えめにしない）
3. メモの料理が写真にも写っている場合のみ、メモの値だけを採用（重複カウント禁止）
4. 写真にのみ写っていて「メモに記載が無い料理」は、写真から推定してitemsに追加する
5. items配列 = 「メモのすべての料理」+「写真にのみある料理」（重複なし）

例1：メモ「白米150g」+ 写真に「白米・味噌汁・唐揚げ」
→ items: [{白米 150g（メモ値）}, {味噌汁 1杯（写真）}, {唐揚げ 3個（写真）}]

例2：メモ「鶏むね肉100g」+ 写真に「チョコケーキ」のみ
→ items: [{鶏むね肉 100g（メモ値）}, {チョコケーキ 1個（写真）}]
→ メモと写真が完全に別物でも、両方を必ず記録する`
      : `\n\n【写真とメモの両方を必ず加味する】
顧客メモ：${supplementText}

このメモは写真の補足情報です。以下を厳守してください：
1. 写真に写っている料理を基本に推定する
2. メモに「写真に映っていない料理」が書かれていれば、items配列に追加する（最重要）
   - 例：写真に丼物だけ→メモ「味噌汁とサラダも食べた」なら、items に味噌汁・サラダを追加
   - 撮り忘れた料理を補完するための文章が来ることが多い
3. メモの修飾表現（大盛り/少なめ/半分/2倍/○○抜き/ノンオイル/低糖質/無糖 等）は標準量から増減して反映する
...
6. 写真とメモが矛盾する場合は、メモを優先する（顧客が実際に食べた本人の申告）`
    : '';
