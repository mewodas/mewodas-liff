...
function Inner() {
  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/admin/reports/generate', {
        ...
        body: JSON.stringify({ customerId, templateId: templateId || undefined, startDate, endDate }),
      });
      if (!res.ok) {
  ...
  async function send() {
    if (!customerId || !title.trim() || !body.trim()) { setError('顧客・タイトル・本文すべて必要'); return; }
    setSending(true);
    setError(null);
    setResultMsg(null);
    try {
      ...
      const res = await fetch('/api/admin/notifications', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customerId, category, title: title.trim(), body: bodyText, staffName: customerStore?.name || '', sendLinePush }),
      });
      if (!res.ok) {
        const j = await res.json().catch(() => null);
      ...
      const j = await res.json();
      if (j?.push?.pushed) { setResultMsg('送信完了（LINEプッシュあり）'); toast.success('送信しました'); }
      else { setResultMsg('保存完了（LINEプッシュ未送信）'); toast.success('保存しました'); }
    } catch (e) {
      setError(e instanceof Error ? e.message : 'エラー');
  ...
  async function submitAnnouncement() {
    if (!annTitle.trim() || !annBody.trim()) { setAnnFormError('タイトルと本文は必須です'); return; }
    setAnnSending(true);
