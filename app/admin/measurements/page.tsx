...
export default function MeasurementsPage() {
  ...
  }, [customers, statusFilter, storeFilter]);

  // 絞り込み変更時に選択中の顧客・フォーム・詳細をリセット（候補外の顧客が残らないように）
  ...
  return (
    <AdminShell title="体組成計測記録">
      <div className="space-y-3">
        {/* 顧客選択 — 進捗管理と同じ絞り込みUI（店舗フィルタ＋顧客プルダウン＋ステータス） */}
        ...
        {!selectedCustomer && !loadingCustomers && (
          <div className="text-center text-stone-500 py-12 bg-white rounded-2xl border border-stone-200 text-sm">
            顧客を選択すると計測記録の登録・履歴が表示されます
          </div>
        )}
