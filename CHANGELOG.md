...
## 2026-05-21 (staging) – fix: オンボーディングツアーのコメント欄の高さを統一

- fix(OnboardingTour): ツールチップの説明文に `min-h` ＋ 上下中央揃え（`flex items-center`）を設定。ステップ毎に説明文の行数（2〜3行）が違っても**コメント欄の高さ・文字位置が揃う**ように。ステップ間で矢印の見え方が短く/長く感じる不揃いを解消
