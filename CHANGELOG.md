...
## 2026-07-05 13:00 claude/sec-fix-1435749
- fix(security): `app/page.tsx` の Open Redirect を修正（`liff.state` を相対パスのみ許可）
- 影響範囲: LIFF トップページのリダイレクト処理のみ。外部 URL へのリダイレクトを拒否し `/home` にフォールバック
- 関連: Slack #security-alerts Search Console 警告調査（2026-07-04）での付随発見
