...
- feat(liff/home): 中央配置のカード型レイアウトに変更（白カード + 警告アイコン + 太字見出し + 補足説明）
- fix(liff/home): /api/today 等が「進行中ではありません」エラーを返した場合のフォールバックも同じ案内画面に統一（食事記録への導線「食事記録へ」リンクを削除）
- 影響範囲: 顧客側 LIFF `/home`（staging のみ）
