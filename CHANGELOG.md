...
## 2026-06-14 – fix(api): /api/chat の顧客未検出時の 404 を graceful fallback（200）に変更
- fix: `app/api/chat/route.ts` — `getCustomerByLineId` が null のとき 404 を返していたバグを修正。`console.error` でログを残しつつ 200 + 親切なメッセージを返すよう変更（exercise-log・weight と同パターン）
- 影響範囲: API（/api/chat）。顧客 UI には影響なし（エラー表示が消え、案内メッセージが表示される）
- 関連: Sentry 週次レポートの継続中 2 件の主因（週 2.1k エラーの一部）
