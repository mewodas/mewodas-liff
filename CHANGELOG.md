...
- feat(seats): `getSeatStatus()` で「進行中」「設定中」だけを席数カウント対象に。「休止中」「卒業」顧客は除外
- feat(billing UI): 使用席数の下に「※ 休止中・卒業の顧客は席数カウント対象外（総顧客 N名）」を表示（除外がある場合のみ）
- 影響範囲: 顧客追加時の席数上限チェック、`/store/billing` 表示、自社席数変更モーダルの safeMin、`/api/stripe/update-seats` の減枠ガード
- 設計: 設定中（オンボード中）は seat 利用とみなしてカウント、本格運用前に一時休止した顧客は課金対象外に
